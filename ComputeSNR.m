function [SNR] = ComputeSNR(signal, fs, annotatedOnsets, annotatedOffsets)
	% ComputeSNR2 - Estimates the SNR given a set of manual annotations in seconds
	%	
	% This algorithm is supposed to estimate the signal-to-noise ratio in recordings that were
	% manually annotated (meaning that the desired and noise segments are known) and where the 
	% noise is completely uncorrelated with the desired signal (additive noise). This 
	% implementation might help in cases were there is neither information regarding the sound
	% intensity (no sound meter was employed), nor the voltage-sound-intensity relationship in
	% devices, nor separated desired and noise signals.
	%
	% The implemented formula (eq. 1) is presented below. P(s) is the power of a finite period 
	% signal and is calculated by donig: P(s) = 1/N * (|s(1)|^2+|s(2)|^2+...+|s(N)|^2)
	%
	%   SNR = SNR = 10 * log10((P(desired)−P(noise))/P(noise))  (eq. 1)
	%
	% The desired signal is constructed by appending the entries of the mixed signal specified 
	% by the annotated segments (onset[i]-offset[i] pair), and the noise signal is constructed 
	% by appending the entries outside the annotations. Good annotations should make the noise 
	% signal contain only silence or not-desired segments.   
	%
	% DEBUG=true saves the segmented signals in two files called "desired.wav" and "noise.wav"
	% and prints a graph with the annotations and the oscillogram of the saved wav files.
	%
	% The algorithm was taken from:
	% 1. Papadopoulos, P., & Tsiartas, A. (2014). A supervised signal-to-noise ratio estimation 
	%    of desired signals. IEEE International Conference on Acoustics, desired and Signal
	%    Processing, 8287–8291
	% 2. Beritelli, F., Casale, S., Grasso, R., & Spadaccini, A. (2010). Performance Evaluation
	%    of SNR Estimation Methods in Forensic Speaker Recognition. In 2010 Fourth International 
	%    Conference on Emerging Security Information, Systems and Technologies (pp. 88–92)
	%
	% Acknowledgements:
	%    I could not finish this script without the help of Msm. Thank you very much. 
	%    http://dsp.stackexchange.com/questions/35041/snr-estimation-in-segmented-desired-signals
	%    
	% Syntax: ComputeSNR2(signal, fs, annotatedOnsets, annotatedOffsets)
	%
	% Inputs:
	%    signal - array containing the mixed signals: noise and desired
	%    fs - sampling rate
